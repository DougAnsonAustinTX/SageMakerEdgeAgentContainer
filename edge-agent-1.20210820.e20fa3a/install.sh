...
echo "Do you wish to clean up existing unix socket if there is any?"
select yn in "Yes" "No"; do
    case $yn in
        Yes ) REPLACE_CLEANUP='-f'; break;;
        No ) REPLACE_CLEANUP=''; break;;
    esac
done
...
sed -i "s@REPLACE_CLEANUP@${REPLACE_CLEANUP}@" ${SYSTEMD_DIR}/sagemaker_edge_agent.service
